...
<div class="container mx-auto h-full flex justify-center items-center text-center">
	<div class="card m-5 p-8 space-y-5 w-full sm:w-1/2 lg:w-1/3">
		...
		<form method="post" action="?/login"  class="space-y-5 p-4" use:focusTrap={true} use:enhance={() => {
			return async ({ update }) => {
			...
			<button type="submit" disabled={buttonDisabled}  class="btn variant-filled w-full font-bold"
				>Login</button
			>
			...
			>
			<button formaction="?/discordLogin" class="logo-item" href="/">
				<span
			...
			<button formaction="?/githubLogin" class="logo-item" href="/">
				<span
